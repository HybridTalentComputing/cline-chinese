						...
						if (didAutoApprove && this.autoApprovalSettings.enableNotifications) {
							...
							timeoutId = setTimeout(() => {
								showSystemNotification({
									subtitle: "命令任在执行",
									message: "自动批准的命令已运行 30 秒，可能需要您的注意。",
								})
							}, 30_000)
						}
			...
			case "new_task": {
				...
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "Cline 需要启动一个新任务...",
								message: `Cline 建议用以下上下文开启一个新任务: ${context}`,
							})
						}
			...
			case "condense": {
				...
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "Cline 想要缩短对话...",
								message: `Cline 建议缩短对话为: ${context}`,
							})
						}
					...
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								...
								message: `Cline 建议创建一个 GitHub issue，标题为: ${title}`,
							})
						}
						...
						if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
							...
							this.pushToolResult(
								formatResponse.toolResult(
									`用户没有提交错误，而是在生成的 Github issue上提供了反馈:\n<feedback>\n${text}\n</feedback>`,
									images,
									fileContentString,
								),
								block,
							...
							// If no response, the user accepted the condensed version
							this.pushToolResult(formatResponse.toolResult(`用户接受了 Github issue的创建.`), block)
			...
			case "attempt_completion": {
				...
				try {
					...
					if (block.partial) {
						...
						toolResults.push({
							type: "text",
							text: `用户已对结果提供反馈。请考虑他们的反馈以继续执行任务，然后再次尝试完成.\n<feedback>\n${text}\n</feedback>`,
						})
