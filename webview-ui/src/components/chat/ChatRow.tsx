			...
			switch (type) {
				...
				case "use_mcp_server":
					...
					return [
						...
						<span
							...
							Cline 需要 {mcpServerUse.type === "use_mcp_tool" ? "使用工具" : "访问资源"} 在{" "}
				...
				case "followup":
					return [
						...
						<span style={{ color: normalColor, fontWeight: "bold" }}>Cline 一个疑问:</span>,
					]
				default:
					return [null, null]
			}
					...
					return (
						<>
							<div style={headerStyle}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "T该文件不在您的工作区内")}
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask" ? "Cline 需要查看上级目录:" : "Cline 查看当前目录文件:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
						...
						<>
							<div style={headerStyle}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "T该文件不在您的工作区内")}
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask" ? "Cline 需要递归检索上层目录:" : "Cline 需要递归检索当前目录:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
					...
					return (
						<>
							<div style={headerStyle}>
								...
								<span style={{ fontWeight: "bold" }}>
									Cline 需要搜索目录 <code style={{ wordBreak: "break-all" }}>{tool.regex}</code>:
								</span>
							</div>
