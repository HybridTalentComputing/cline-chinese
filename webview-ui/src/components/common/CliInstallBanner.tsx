	...
	return (
		<div
			...
			<h4 className="m-0 flex items-center gap-2" style={{ paddingRight: "24px" }}>
				...
				{isMacOSOrLinux() ? "Cline for CLI 来了！" : "Cline CLI 功能"}
			</h4>
			<p className="m-0">
				{isMacOSOrLinux() ? (
					<>
						安装 Cline 即可直接在终端中使用，并启用子代理功能。Cline 可以生成 cline
						命令来处理诸如浏览大型代码库以获取信息之类的特定任务。这样，通过在单独的子进程中运行这些操作，可以保持主上下文窗口的简洁。{" "}
						<a
					...
					<>
						Cline CLI 现已支持 macOS 和 Linux！即将支持其他平台。{" "}
						<a
							href="https://docs.cline.bot/cline-cli/overview"
							rel="noopener noreferrer"
							style={{ color: "var(--vscode-textLink-foreground)" }}
				...
				{isMacOSOrLinux() ? (
					...
					<div className="flex gap-2">
						...
						<VSCodeButton
							...
							className="flex-1"
							disabled
							title="Cline CLI 和子代理仅适用于 macOS 和 Linux 系统。">
							子 agents (即将支持 Windows)
						</VSCodeButton>
					</div>
				)}
			</div>
