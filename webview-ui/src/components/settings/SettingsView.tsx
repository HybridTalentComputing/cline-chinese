	...
	return (
		<Tab>
			...
			<UnsavedChangesDialog
				...
				title={pendingModeSwitch ? "保存更改?" : "不保存更改"}
				description={
					pendingModeSwitch
						? `是否要将更改保存到 ${chatSettings.mode === "plan" ? "计划" : "执行"} 模式在切换到 ${pendingModeSwitch === "plan" ? "计划" : "执行"} 模式之前?`
						: "您有未保存的更改。您确定要放弃它们吗？"
				}
				confirmText={pendingModeSwitch ? "切换而不保存" : "放弃更改"}
				saveText="保存并切换"
