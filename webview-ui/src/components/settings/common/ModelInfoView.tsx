...
export const ModelInfoView = ({ selectedModelId, modelInfo, isPopup }: ModelInfoViewProps) => {
	...
	const inputPriceElement = hasTiers ? (
		...
		<span key="inputPrice">
			<span style={{ fontWeight: 500 }}>输入:</span> {formatTokenPrice(modelInfo.inputPrice)}
		</span>
	) : null
	...
	if (hasThinkingConfig && modelInfo.outputPrice !== undefined && modelInfo.thinkingConfig?.outputPrice !== undefined) {
		...
		outputPriceElement = (
			<Fragment key="outputPriceConditional">
				<span style={{ fontWeight: 500 }}>输出 (标准):</span> {formatTokenPrice(modelInfo.outputPrice)}
				<br />
				<span style={{ fontWeight: 500 }}>输出 (思考 &gt; 0):</span>{" "}
		...
		outputPriceElement = (
			<Fragment key="outputPriceTiers">
				...
				<span style={{ fontStyle: "italic" }}> (基于输入 tokens 数)</span>
				<br />
		...
		outputPriceElement = (
			<span key="outputPrice">
				<span style={{ fontWeight: 500 }}>输出:</span> {formatTokenPrice(modelInfo.outputPrice)}
			</span>
		)
	}
	...
	const infoItems = [
		...
		modelInfo.contextWindow !== undefined && modelInfo.contextWindow > 0 && (
			<span key="contextWindow">
				<span style={{ fontWeight: 500 }}>上下文窗口:</span> {formatTokenLimit(modelInfo.contextWindow)} tokens
			</span>
		),
		inputPriceElement, // Add the generated input price block
		modelInfo.supportsPromptCache && modelInfo.cacheWritesPrice && (
			<span key="cacheWritesPrice">
				<span style={{ fontWeight: 500 }}>写缓存:</span> {formatTokenPrice(modelInfo.cacheWritesPrice || 0)}
			</span>
		),
		modelInfo.supportsPromptCache && modelInfo.cacheReadsPrice && (
			<span key="cacheReadsPrice">
				<span style={{ fontWeight: 500 }}>读缓存:</span> {formatTokenPrice(modelInfo.cacheReadsPrice || 0)}
			</span>
		),
