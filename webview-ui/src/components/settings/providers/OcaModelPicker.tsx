...
const OcaModelPicker: React.FC<OcaModelPickerProps> = ({
	...
	return (
		<div className="w-full">
			...
			`}</style>
			<label className="font-medium text-[12px] mt-[10px] mb-[2px]">{t("settings.apiConfig.oca.model")}</label>
			<div className="relative z-100 flex items-center gap-2 mb-1">
				...
				<VSCodeButton
					...
					{loading ? t("settings.apiConfig.oca.refreshing") : t("settings.apiConfig.oca.refresh")}
				</VSCodeButton>
			</div>
			{lastRefreshedText ? (
				<div className="text-[11px] text-(--vscode-descriptionForeground) mt-0 mb-2">
					{t("settings.apiConfig.oca.lastRefreshedAt", { time: lastRefreshedText })}
				</div>
			) : null}
			{selectedModelInfo && (
				<>
		...
		<div className="fixed top-0 left-0 w-screen h-screen z-2000 [background:rgba(0,0,0,0.25)] flex items-center justify-center">
			<div
				...
				<h2 className={`mt-0 [color:var(${VSC_FOREGROUND},#111)] font-bold`} id="oca-popup-title">
					{t("settings.apiConfig.oca.acknowledgementRequired")}
				</h2>
				<h4 className={`mb-2 [color:var(${VSC_DESCRIPTION_FOREGROUND},#b3b3b3)] font-semibold`}>
					{t("settings.apiConfig.oca.disclaimer")}
				</h4>
				...
				<div className="text-right">
					<VSCodeButton
						...
						type="button">
						{t("settings.apiConfig.oca.acknowledgeAndAgree")}
					</VSCodeButton>
				</div>
			</div>
		</div>
